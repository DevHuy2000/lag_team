...
try:
    ...
    logging.error("LỖI: Thiếu file MajorLg.py hoặc ff.py")
...
# --- Main Client Class (Sửa lại kế thừa Thread để chạy nền) ---
...
# --- Danh sách tài khoản (Hardcoded từ file gốc) ---
accounts = [
    {"id": "4320544587", "pass": "BY_PARAHEX-UH3QLBSQA-REDZED", "ip": "103.108.103.30", "port": 39699},
    {"id": "4320544646", "pass": "BY_PARAHEX-RKDCTOFL7-REDZED", "ip": "103.108.103.30", "port": 39699}
#    {"id": "4320544705", "pass": "BY_PARAHEX-IXHVECZQM-REDZED", "ip": "103.108.103.26", "port": 39699}
]
...
def trigger_lag():
    ...
    if not code:
        return jsonify({"status": "error", "message": "Thiếu parameter 'code'"}), 400
    
    try:
        # Chuyển đổi code sang int như logic gốc
        team_code = int(code) 
    except ValueError:
    ...
    # Chạy thread cho từng tài khoản
    active_threads = []
    for acc in accounts:
        t = FFClient(acc['id'], acc['pass'], team_code, acc['ip'], acc['port'])
        t.daemon = True # Thread sẽ tự tắt khi app tắt, nhưng trên server web nó sẽ chạy ngầm
        t.start()
        active_threads.append(acc['id'])
        time.sleep(0.5) # Delay nhỏ để tránh spam request đăng nhập cùng lúc
...
if __name__ == '__main__':
    # Chạy trên local cổng 5000 (mặc định Flask)
